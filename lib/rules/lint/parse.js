...
module.exports = exports = function(payload, entry, fn) {
  ...
  Lint.prettify(entry.content, function(err, formattedContent) {
    ...
    for(var i = 0; i < (result.messages || []).length; i++) {
      ...
      lineStart = payload.getSnippetManager().getStart(lines.length, (currentLine - 1), 3);
      lineEnd = payload.getSnippetManager().getEnd(lines.length, (currentLine - 1), 3);

      // slice and find the snippet
      codeLines = payload.getSnippetManager().slice(lines, lineStart, lineEnd);
      ...
      // check the column
      if(entry.col) occurence.col = 1 * entry.col;
      ...
      if(matches) {
        for(var a = 0; a < matches.length; a++) {
          ...
          occurence.identifiers.push(matches[a].replace(/\'/gi, "").replace(/'\\\''/gi, ""))
      ...
      if(matches) {
        for(var a = 0; a < matches.length; a++) {
          ...
          occurence.identifiers.push( identifier );
